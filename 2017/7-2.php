...
foreach (explode("\n", $input) as $line) {
    ...
    $parsed[$matches[1]] = [
        'w' => $matches[2],
        'tw' => 0,
        'c' => isset($matches[4]) ? explode(',', str_replace(' ', '', $matches[4])) : []
    ];
}
reset($parsed);
$parentIndex = findParent(key($parsed), $parsed);

$tree = [];
$tree[$parentIndex] = buildTree($parentIndex, $parsed);

$tree = fillWeights($tree[$parentIndex]['c']);
findCulprit($tree);


function findParent($name, $tree)
{
    foreach ($tree as $branchName => $branch) {
        if (in_array($name, $branch['c'])) {
            return findParent($branchName, $tree);
        }
    }
    return $name;
}
...
function buildTree($root, $tree)
{
    $rootNode = $tree[$root];
    $children = [];
    if (count($tree[$root]['c']) > 0) {
        foreach ($rootNode['c'] as $newRoot) {
            $children[$newRoot] = buildTree($newRoot, $tree);
        }
    }
...
function fillWeights($tree)
{
    foreach (array_keys($tree) as $nodeIndex) {
        $totalWeight = $tree[$nodeIndex]['w'];
        $tree[$nodeIndex]['c'] = fillWeights($tree[$nodeIndex]['c']);
        foreach ($tree[$nodeIndex]['c'] as $childNode) {
            $totalWeight += $childNode['tw'];
        }
        $tree[$nodeIndex]['tw'] = $totalWeight;
    }

    return $tree;
}

function findCulprit($tree) {
    $weights = [];
    $candidates = [];
    if (count($tree) < 3) {
        $candidates = $tree;
    ...
    foreach ($candidates as $candidate) {
        findCulprit($candidate['c']);
    }
    if (count($candidates) === 1 && $compare !== null) {
        echo $candidate['w'] - ($candidate['tw'] - $compare) . "\n\n";die;
    }
}
