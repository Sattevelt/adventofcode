...
//var_dump($coords);
$intersections = findIntersectionsWithDistance($coords[0], $coords[1]);
var_dump(array_key_first($intersections));

function pathToCoords(string $path) {
    $coords = [];
    $curX = 0;
    $curY = 0;
    $pathElems = explode(',', $path);
    foreach ($pathElems as $pathElem) {
        $matches = [];
        if (preg_match('/^(U|D|L|R)([0-9]+)$/', trim($pathElem), $matches) === 1) {
...
function findIntersectionsWithDistance(array $coords1, array $coords2) {
    $intersections = [];
    foreach ($coords1 as $coord1) {
        foreach ($coords2 as $coord2) {
            if ($coord1[0] == $coord2[0] && $coord1[1] == $coord2[1]) {
                $dist = abs($coord1[0]) + abs($coord1[1]);
                $intersections[$dist] = $coord1;
            }
        }
    }
    ksort($intersections);
