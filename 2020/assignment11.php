...
$versions = [];
$count = 1;
$newData = getNextIteration($data);
while (! in_array($newData, $versions)) {
    $versions[] = $newData;
//    outputGrid($newData);
    echo "Iterating $count     \n";
    $newData = getNextIteration($newData);
    echo "\rComparing $count     \n";
    $count++;
}
echo "\n";
outputGrid($newData);
...
function outputGrid(array $data) {
...
function getNextIteration(array $data) {
    $newData = $data;
    foreach ($data as $y => $row) {
        foreach ($row as $x => $seat) {
            ...
            if ($curValue === '.') {
                ...
                $numSeats = getNumVisibileSeats($data, $x, $y);
                if ($curValue === 'L' && $numSeats === 0) {
                    $newValue = '#';
                } elseif ($curValue === '#' && $numSeats >= 5) {
                    $newValue = 'L';
                } else {
                    $newValue = $curValue;
                }
...
function getNumVisibileSeats(array $data, int $x, int $y) {
    $count = 0;
    $xMods = [0, 1, -1];
    $yMods = $xMods;
...
//            echo "** $xMod || $yMod ** \n";
            ...
            do {
                $newX = $newX + $xMod;
                $newY = $newY + $yMod;
                $value = !isset($data[$newY][$newX]) ? '' : $data[$newY][$newX];
//                echo "$newX, $newY: $value\n";
            } while ($value === '.');
