...
$lineRegex = '/^((mask = ((0|1|X){36}))|(mem\[([0-9]+)\] = ([0-9]+)))$/';
$mask = '';
$values = [];


//$mask = '00000000000000000000000000000000X0XX';
//echo str_pad(decbin(26), 36, '0', STR_PAD_LEFT) . " = 26\n";
//echo $mask . "\n\n";
//
//foreach (applyMask(26, '00000000000000000000000000000000X0XX') as $value) {
//    echo str_pad(decbin($value), 36, '0', STR_PAD_LEFT) . " = $value\n";
//};
//die;

foreach ($data as $line) {
    $matches = [];
    preg_match($lineRegex, $line, $matches);
//    echo "line $line: ";
    if ($matches[3] != '') {
        $mask = $matches[3];
//        echo "mask parsed\n";
    } else {
        $address = (int)$matches[6];
        $value = (int)$matches[7];

        $appliedAddresses = applyMask($address, $mask);
        foreach ($appliedAddresses as $appliedAddress) {
            $values[$appliedAddress] = $value;
        }
    }
}
...
function applyMask(int $value, string $mask) {
    $valueStr = str_pad(decbin($value), 36, '0', STR_PAD_LEFT);
    $maskArray = str_split($mask);
    $applied = '';
    foreach ($maskArray as $index => $bitValue) {
        switch ($bitValue) {
            case 'X':
            case '1':
                $applied .= $bitValue;
                break;
            case '0':
                $applied .= substr($valueStr, $index, 1);
                break;
        }
    }
    return getBitMasks($applied, []);
}


function getBitMasks(string $mask, array $masks) {
    $index = stripos($mask, 'X');
    if (false === $index) {
        // No replacements left. Convert to int and add to masks
        $masks[] = bindec($mask);
        return $masks;
    } else {
        $mask1 = substr_replace($mask, '1', $index, 1);
        $mask2 = substr_replace($mask, '0', $index, 1);

        return getBitMasks($mask1, getBitMasks($mask2, $masks));
    }
}