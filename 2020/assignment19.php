...
$state = 'rules';
$ruleRegex = "/^([0-9]+)\: (.*)$/";
$nextIterationSubjects = [];
foreach ($data as $line) {
    if ($line === '8: 42') {
        $line = '8: ( 42 )+';
    } elseif ($line === '11: 42 31')
        $line = '11: ( 42 ){n}( 31 ){n}';
    if ($line === '') {
        $state = 'messages';
        continue;
    ...
    if ($state === 'rules') {
        $matches = [];
        preg_match($ruleRegex, $line, $matches);
        $key = (int)$matches[1];
        $value = trim($matches[2], ' "');
        if (strlen($value) === 1) {
            $nextIterationSubjects[$key] = $value;
            $rules[$key] = $value;
        } else {
            $rules[$key] = explode(' ', $value);
        }
    } elseif ($state === 'messages') {
        $messages[] = $line;
    }
}

$newNextIterationSubjects = $nextIterationSubjects;
while (count($newNextIterationSubjects) > 0) {
    $nextIterationSubjects = [];

    foreach ($rules as $key => $value) {
        if (array_key_exists($key, $newNextIterationSubjects)) {
            continue;
        ...
        foreach ($value as $valueKey => $valuePart) {
            if (array_key_exists($valuePart, $newNextIterationSubjects)) {
                $rules[$key][$valueKey] = $newNextIterationSubjects[$valuePart];
            } elseif ($nonNumeric && is_numeric($valuePart)) {
                $nonNumeric = false;
            }
        }

        if ($nonNumeric) {
            $newValue = '(' . implode('', $rules[$key]) . ')';
            $rules[$key] = $newValue;
            $nextIterationSubjects[$key] = $newValue;
        }
    }
    $newNextIterationSubjects = $nextIterationSubjects;
}
...
foreach ($messages as $message) {
    for ($i = 1; $i <= 10; $i++) {
        $regex = sprintf('/^%s%s$', $rules[8], str_replace('n', $i, $rules[11]));
