...
$allergens = [];
$ingredients = [];
$regex = '/^(([a-z]+ )+)\(contains ([a-z]+(, [a-z]+)*)\)$/';
// Parse line. Keep track of every ingredient mentioned and store allergens with possible ingredients
foreach ($data as $line) {
    $matches = [];
    if (preg_match($regex, $line, $matches) === 1) {
        $lineIngredients = explode(' ', trim($matches[1]));
        $lineAllergens = explode(', ', $matches[3]);

        foreach ($lineIngredients as $lineIngredient) {
            $ingredients[] = $lineIngredient;
        }
        foreach ($lineAllergens as $lineAllergen) {
            if (! array_key_exists($lineAllergen, $allergens)) {
                $allergens[$lineAllergen] = $lineIngredients;
            } else {
                $allergens[$lineAllergen] = array_intersect($allergens[$lineAllergen], $lineIngredients);
            }
        }
    } else {
...
// Reduce list of ingredents for allergens until every allergen has exactly one ingredient.
$updated = true;
while ($updated) {
    $updated = false;
    foreach ($allergens as $allergen => $ingredientCandidates) {
        if (!is_array($ingredientCandidates) || count($ingredientCandidates) != 1) {
            continue;
        }
        $value = reset($ingredientCandidates);

        $allergens[$allergen] = $value;
        foreach ($allergens as $allergen2 => $ingredientCandidates2) {
            if (!is_array($ingredientCandidates2)) {
                continue;
            }
            if (($key = array_search($value, $ingredientCandidates2)) !== false) {
                unset($allergens[$allergen2][$key]);
            }
        }
        $updated = true;
    }
...
// Can hold duplicate ingredients if they are mentioned multiple times.
$safeIngredients = [];
...
echo "Number of mentions of safe ingredients: " . count($safeIngredients) . "\n";
ksort($allergens);
print_r($allergens);
echo "Canonical dangerous ingredients: " . implode(',', $allergens) . "\n";
