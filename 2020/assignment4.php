...
while ($i < $lines) {
    ...
    foreach ($parts as $part) {
        ...
        if (preg_match('/^([a-z]{3})\:(.*)$/', $part, $matches)) {
            $parsedDataLine[$matches[1]] = $matches[2];
        }
    }
    $i++;
}
...
foreach ($parsedData as $passportData) {
    foreach ($mustContain as $key) {
        if (! array_key_exists($key, $passportData)) {
            echo "invalid by missing key: $key\n";
            continue 2;
        }
        $value = $passportData[$key];
    ...
    switch ($key) {
        ...
        case 'hgt':
            ...
            if (preg_match('/^([0-9]+)(cm|in)$/', $value, $matches) === 1) {
                $num = (int)$matches[1];
                if ($matches[2] == 'cm' && 150 <= $num && $num <= 193) {
                    $valid = true;
                } elseif ($matches[2] == 'in' && 59 <= $num && $num <= 76) {
                    $valid = true;
                } else {
                    $valid = false;
                }
            ...
            break;
        case 'hcl':
            $valid = preg_match('/^\#[0-9a-f]{6}$/', $value) === 1;
            break;
        case 'ecl':
            $valid = preg_match('/^(amb|blu|brn|gry|grn|hzl|oth)$/', $value) === 1;
            break;
        case 'pid':
            $valid = preg_match('/^[0-9]{9}$/', $value) === 1;
            break;
    }
if (!$valid) {
    echo "invalid by: $key - $value\n";
}
    return $valid;
}
