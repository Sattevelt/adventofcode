...
foreach ($lines as $line) {
    ...
    if (count($matches) !== 7) {
        throw new Exception('No regex match for line: ' . $line);
    }
...
foreach ($parsed as $line) {
    // Check if first. If false we don't have to parse the rest.
    $ifBankVal = $banks[$line['ifbank']];
    $ifValue = $line['ifval'];
    $ifOp = $line['ifop'];

    switch ($ifOp) {
        case '<':
            $result = $ifBankVal < $ifValue;
            break;
        case '<=':
            $result = $ifBankVal <= $ifValue;
            break;
        ...
        case '!=':
            ...
            break;
        default:
            throw new Exception('Illegal if operator for line ' . implode(' ', $line));
    }
    ...
    if ($line['op'] == 'inc') {
        $banks[$line['tbank']] += $line['val'];
    } else {
        $banks[$line['tbank']] -= $line['val'];
    }
    $maxValue = max($maxValue, $banks[$line['tbank']]);
}

arsort($banks);
var_dump(reset($banks), $maxValue);
