...
function parseString(string $string)
{
    ...
    $regex = "/\((.*)\)/";
    while (preg_match($regex, $string, $matches) === 1) {
        $string = parseParentheses($string);
    }
    // Tokenize into array of numbers and operators
    $tokens = tokenize($string);

    // Process operators in order of precedence
    $operators = ['+', '*'];
    foreach ($operators as $operator) {
        while (($key = array_search($operator, $tokens)) !== false) {
            // Get surrounding numbers and calculate answer.
            $num1 = $tokens[$key - 1];
            $num2 = $tokens[$key + 1];
            $answ = 0;
            switch ($operator) {
            ...
            // Replace the 3 used elements with answer
            array_splice($tokens, $key - 1, 3, $answ);
        }
    }
    return reset($tokens);
}
...
function tokenize(string $string)
{
    ...
    for ($i = 0; $i < $length; $i++) {
        ...
        if (is_numeric($char)) {
            if ($i + 1 < $length && is_numeric($string[$i + 1])) {
                continue;
            }
            $tokens[] = (int)$char;
        } else {
...
function parseParentheses(string $string)
{
    $count = 0;
    $running = false;
    $start = 0;
    $pos = 0;
    $length = strlen($string);
    while (($count > 0 || $running === false) && $pos < $length) {
    ...
    if ($running) {
        $string = substr_replace($string, parseString(substr($string, $start + 1, $pos - $start - 2)), $start, $pos - $start);
    }
