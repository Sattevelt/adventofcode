...
foreach ($data as $row) {
    ...
    $groupData[] = str_split($row);
}
$groupsData[] = $groupData;

foreach ($groupsData as $key => $groupData) {
    $commonAnswers = null;
    foreach ($groupData as $personAnswers) {
        if ($commonAnswers === null) {
            $commonAnswers = $personAnswers;
        } else {
            $commonAnswers = array_intersect($commonAnswers, $personAnswers);
        }
    }
    $groupsData[$key] = count($commonAnswers);
}

echo array_sum($groupsData) . "\n";