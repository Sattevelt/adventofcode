...
// Parse data
$parsed = [];
$lineRegex = '/^([a-z\ ]+) bags contain (no other bags|[0-9a-z\,\ ]+)\.$/';
$containRegex = '/^([0-9]+) ([a-z\ ]+) bag(s)?/';
foreach ($data as $row) {
    $matches = [];
    if (preg_match($lineRegex, $row, $matches) === 1) {
        $key = $matches[1];
        $parsed[$key] = [];
        if ($matches[2] !== 'no other bags') {
            ...
            foreach ($parts as $part) {
                $matches = [];
                if (preg_match($containRegex, trim($part), $matches) === 1) {
                    $parsed[$key][$matches[2]] = $matches[1];
                }
            }
        }
    }
...
$newChild = 'shiny gold';
echo getCount($newChild) . "\n";

function getCount($childKey) {
    global $parsed;

    $total = 0;
    if (empty($parsed[$childKey])) {
        return 0;
    }

    foreach ($parsed[$childKey] as $newChildKey => $quantity) {
        $total += $quantity; // Add the bag it self
        $total += $quantity * getCount($newChildKey); // Add bags content
    }
