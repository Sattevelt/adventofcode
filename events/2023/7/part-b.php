...
function solvePuzzle(array $data): int
{
    ...
    foreach ($hands as $key => $hand) {
        ...
        $totalScore += $winning;
        $handScore = ((string)$hand->getScore())[0];
        echo sprintf('%s: (%s) %s x %s => %s' . PHP_EOL, $hand->getHand(), $handScore, $multiplier, $bet, $winning);
    }
...
//        'J' => '11',
    ...
    public function __construct(string $hand, int $bet)
    {
        ...
        $sortedCards2 = $sortedCards;
        rsort($sortedCards, SORT_NUMERIC);
        arsort($sortedCards2, SORT_NUMERIC);
        // Apply the jokers if there is at least one, and not all jokers.
        if (array_key_exists('J', $sortedCards2) && count($sortedCards2) > 1) {
            $nth = null; // Which index are the jacks?
            foreach (array_keys($sortedCards2) as $key => $card) {
                if ($card === 'J') {
                    $nth = $key;
                    break;
                }
            }
            $sortedCards[$nth === 0 ? 1 : 0] += $sortedCards2['J'];
            unset($sortedCards[$nth]);
            rsort($sortedCards);
        }
